...
def eval_metrics(pred_traj, gt_traj, sample_stride=1):
    assert (
        sample_stride == 1 and gt_traj is not None
    ), "Align with Monster, stride=1, gt_traj is not None"
    # pred_traj [(N,7), (N) float64 range(N)]
